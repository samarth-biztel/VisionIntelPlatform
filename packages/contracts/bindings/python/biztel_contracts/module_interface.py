...
Owner: Chetak. This is the contract AI Supervisor, Image Inspection, OCR, and
Crowning are built against. Python binding of ``src/module-interface.js``, plus
the abstract base class the JS side has no need for.

Two invariants this file exists to enforce (MEMORY.md):

* **A module never contains GPU machinery.** It *calls* the Vision Runtime.
  Modules that load their own engines produce multiple CUDA contexts on one
  Jetson and OOM. Machinery is Layer 1; meaning is Layer 2 -- this class is
  Layer 2 only.
* **No module calls another module directly.** A module is handed a publish
  callback and its input topics; it is given no way to reach another service.
...
class ModuleProvides(BaseModel):
    ...
    def _check_result_topics(cls, value: List[str]) -> List[str]:
        for topic in value:
            # You publish to a concrete topic, never a wildcard.
...
class Module(ABC):
    ...
    A module subscribes, processes, and publishes. It is handed exactly two
    things -- its config and a publish callback -- and has no route to the core's
    internals, the bus itself, or any other module.
        ...
        class MyModule(Module):
            ...
            def on_frame(self, frame):
                return self.make_result(
                    frame, verdict="pass", payload={"coverage": 0.82}
                )
    """

    #: Every concrete module must declare its manifest.
    manifest: ModuleManifest
    ...
    def on_frame(self, frame: FrameEnvelope) -> Optional[ResultEnvelope]:
        ...
        Return a :class:`ResultEnvelope` to publish it, or ``None`` to produce
        nothing for this frame (a module decides *when* to infer -- Layer 2).
        """
    ...
    def make_result(
        ...
        Fills in the correlation fields (frame_id, source_topic, result_topic,
        module_id) so a module cannot accidentally break traceability.
        """
