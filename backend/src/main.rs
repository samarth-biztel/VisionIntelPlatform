...
static SHARED_STATE: OnceLock<Arc<Mutex<RuntimeState>>> = OnceLock::new();
...
fn parse_simple_yaml(raw: &str) -> Value {
    ...
    while index < lines.len() {
        ...
        if !value.is_empty() {
            root.insert(key, parse_scalar(value));
            index += 1;
            continue;
        }
                ...
                if child_value.is_empty() {
                    ...
                    while index < lines.len() && lines[index].starts_with("    ") {
                        if let Some(item) = lines[index].trim().strip_prefix("- ") {
                            nested_array.push(parse_scalar(item));
                        }
                        index += 1;
                    }
    ...
    T: for<'de> Deserialize<'de>,
{
    ...
    for extension in ["yaml", "yml", "json"] {
        ...
        if path.exists() {
            ...
            let json_value = if extension == "json" {
...
fn load_config() -> PlatformConfig {
    let default_device = DeviceConfig {
        device_id: "dev-workstation".to_string(),
        role: default_role(),
        runs: default_runs(),
        bus: BusConfig::default(),
    };
    let default_site = SiteConfig {
        site_id: "biztel-lab".to_string(),
...
fn service(
    ...
    ServiceRecord {
        schema_version: "service-registration.v1".to_string(),
        service_id: id.to_string(),
        display_name: display.to_string(),
        role: role.to_string(),
        version: "0.1.0".to_string(),
        host_id: "dev-workstation".to_string(),
        process_id: None,
        subscribes: subscribes.iter().map(|s| s.to_string()).collect(),
        publishes: publishes.iter().map(|s| s.to_string()).collect(),
        health_topic: "health.heartbeat".to_string(),
        started_at_utc: now(),
        metadata: BTreeMap::new(),
        state: "running".to_string(),
        stopped_reason: None,
        last_heartbeat: Some(heartbeat(id, "running", None)),
        alive: true,
        heartbeat_status: "fresh".to_string(),
...
fn initial_state() -> RuntimeState {
    ...
    services.insert(
        ...
        service(
            ...
            "Core Supervisor",
            "core",
            vec!["event.service.registered"],
            vec!["health.heartbeat"],
        ),
    );
    services.insert(
    ...
    services.insert(
        "log-sink".to_string(),
        service("log-sink", "Log Sink", "sink", vec![], vec!["result.*"]),
    );
    ...
    RuntimeState {
        ...
        subscribers: vec![
            SubscriptionSummary {
                ...
                topics: vec!["camera.line1".to_string()],
                delivered: 0,
                failed: 0,
            },
            SubscriptionSummary {
                service_id: "log-sink".to_string(),
                topics: vec!["result.*".to_string()],
                delivered: 0,
                failed: 0,
            },
        ],
...
fn publish(state: &mut RuntimeState, topic: &str, payload: Value) -> BusMessage {
    let delivered = state
        ...
        .sum();
    let entry = BusMessage {
        id: format!("{}-{}", state.messages.len() + 1, state.frame_sequence),
        topic: topic.to_string(),
        family: topic_family(topic).unwrap_or(topic).to_string(),
        timestamp_utc: now(),
        message: json!({ "topic": topic, "payload": payload }),
...
fn lifecycle_plan(state: &RuntimeState, reverse: bool) -> Vec<Value> {
    ...
    services.sort_by_key(|service| {
        order
            .iter()
            .position(|role| role == &service.role)
            .unwrap_or(99)
    });
    if reverse {
        services.reverse();
...
fn shared_state() -> &'static Arc<Mutex<RuntimeState>> {
    SHARED_STATE.get_or_init(|| Arc::new(Mutex::new(initial_state())))
}

pub fn handle_api_request(method: &str, path: &str) -> ApiResponse {
    let path = path.split('?').next().unwrap_or("/api/health");
    if method == "OPTIONS" {
        return ApiResponse {
            status_code: 204,
            body: "{}".to_string(),
    ...
    let value = match (method, path) {
        ("GET", "/api/health") => {
            json!({ "status": "ok", "service": "core", "timestamp_utc": now() })
        }
        ("GET", "/api/config") => json!(guard.config),
        ("GET", "/api/contracts") => {
            json!({ "tier0": ["frame-envelope.v1", "result-envelope.v1", "service-registration.v1", "service-heartbeat.v1", "module-manifest.v1", "service-shutdown.v1"], "topics": canonical_topics() })
        }
        ("GET", "/api/dashboard-summary") => dashboard(&guard),
        ("GET", "/api/services") => {
            json!({ "services": guard.services.values().collect::<Vec<_>>() })
        }
        ...
        ("GET", "/api/lifecycle/shutdown-plan") => {
            json!({ "services": lifecycle_plan(&guard, true) })
        }
        ("POST", "/api/lifecycle/startup") => {
            for service in guard.services.values_mut() {
                service.state = "running".to_string();
                service.alive = true;
                service.stopped_reason = None;
            }
            json!({ "services": lifecycle_plan(&guard, false) })
        }
        ("POST", "/api/lifecycle/shutdown") => {
            for service in guard.services.values_mut() {
                service.state = "stopped".to_string();
                service.alive = false;
                service.stopped_reason = Some("operator request".to_string());
            }
            json!({ "services": lifecycle_plan(&guard, true) })
        }
        ("POST", "/api/source/capture") => capture(&mut guard),
        ("GET", "/api/sink/log") => {
            json!({ "sink_id": "log-sink", "count": guard.sink_entries.len(), "entries": guard.sink_entries.iter().take(10).collect::<Vec<_>>() })
        }
        ("GET", "/api/bus") => bus_snapshot(&guard),
        ("GET", "/api/bus/messages") => {
            json!({ "messages": guard.messages.iter().take(25).collect::<Vec<_>>() })
        }
        ("GET", "/api/bus/subscriptions") => json!({ "subscriptions": guard.subscribers }),
        ("POST", "/api/bus/publish") => {
            json!({ "message": publish(&mut guard, "event.service.registered", json!({})) })
        }
        ("POST", "/api/validate/frame") | ("POST", "/api/validate/result") => json!({ "ok": true }),
        _ if method == "POST"
            && path.starts_with("/api/services/")
            && path.ends_with("/shutdown") =>
        {
            json!({ "service": null, "shutdown_plan": lifecycle_plan(&guard, true) })
        }
        _ => {
            return ApiResponse {
                status_code: 404,
                body: serde_json::to_string(&json!({ "error": "not_found" })).unwrap(),
            }
        }
    };
...
fn handle_request(_state: &Arc<Mutex<RuntimeState>>, request: &str) -> Vec<u8> {
    let first = request.lines().next().unwrap_or("GET /api/health HTTP/1.1");
    let mut parts = first.split_whitespace();
    let method = parts.next().unwrap_or("GET");
    let path = parts.next().unwrap_or("/api/health");
    let response = handle_api_request(method, path);
    let status = status_text(response.status_code);
    format!("HTTP/1.1 {status}\r\nContent-Type: application/json\r\nAccess-Control-Allow-Origin: *\r\nAccess-Control-Allow-Headers: content-type\r\nAccess-Control-Allow-Methods: GET,POST,OPTIONS\r\nContent-Length: {}\r\n\r\n{}", response.body.len(), response.body).into_bytes()
}
