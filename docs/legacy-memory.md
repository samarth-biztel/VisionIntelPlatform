...
this first. Everything here is either a settled invariant or an environment fact â€” no speculation, no
work-in-progress.

For the reasoning behind any of it: [PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) (why) Â·
[ARCHITECTURE.md](ARCHITECTURE.md) (how) Â· [CONTEXT.md](CONTEXT.md) (current state, build order, tracker).
...
|---|---|---|
| [MEMORY.md](MEMORY.md) | This file â€” durable facts, invariants, environment | Always, first |
...
| [CONTEXT.md](CONTEXT.md) | Settled / exploratory / open, build order Tier 0â€“5, reuse inventory, pages, frameworks, tracker state + known issues | Picking up work, or onboarding |
...
> Vision Runtime does the GPU work for those that need it â€” and everyone speaks only in topics defined
> by contracts.**
...
- **Chetak** â€” CV / AI / ML: inference, models, image processing, vision runtime, classical CV.
- **Samarth** â€” Web / App: UI, bus/core, sinks, integrations, config, infra frameworks.
...
- **"Both"** in the tracker = AI Supervisor **and** Image Inspection â€” the two products currently
  driving platform requirements.
- **AI Supervisor is hardware-bound and real-time** â†’ must run on-prem. **Crowning is stateless
  request/response** â†’ could run anywhere, including cloud.
- **AI Supervisor's model is X3D-S, trained on a small, imbalanced dataset** â†’ sensitive to input
  distribution shift. This is why camera swaps aren't automatically free.

---

## Invariants â€” violating any of these breaks the design
...
2. **A module never contains GPU machinery â€” it calls the Vision Runtime.** Machinery (engine
   ...
   engines produce multiple CUDA contexts on one Jetson â†’ OOM. A module *declares* the model it needs
   via `requires.models` in its manifest; it never loads one. (This is why "model loading per module"
   was deleted â€” see [ARCHITECTURE.md](ARCHITECTURE.md) Â§5.1.)
3. **No module calls another module directly.** Ever. Everything goes over the bus.
4. **The core never *interprets* a frame.** Core owns the frame **transport** â€” shared-memory
   segments, slot allocation, lifetime, and fan-out (settled 2026-07-28) â€” but never decodes,
...
6. **Sources normalize before publishing** â€” format *and* geometry/calibration â€” so every module sees
...
9. **The inference contract is the version firewall.** It is simultaneously the moduleâ†”runtime interface
...
**Cost of an OS switch, three tiers:** T1 module logic never changes Â· T2 environment rebuilt once per OS
in a Dockerfile (the real cost, paid once) Â· T3 small localized code, i.e. TRT8â†’TRT10 in one backend file.

The two things that cost real time on JP5â†’JP6: finding the JP6 wheels, and the TRT8â†’TRT10 API change.

---

## Default language per role

**Rust** (core/platform â€” supervisor *and* the shared-memory hot path, which Core owns) Â· C++ or Python
...
us** â€” prototype it before committing the UI stack, or consider Qt.

---

## Accepted limits â€” don't rediscover these as bugs
...
- **`openpyxl` is not installed** (pandas 2.0.3 is, but can't read xlsx without it). To read the
  ...
  `sharedStrings.xml`** â€” cell text is stored as `t="inlineStr"` inside `<is><t>`, so a parser that only
...
interface. They unblock all parallel work. Full build order: [CONTEXT.md](CONTEXT.md) Â§6.
