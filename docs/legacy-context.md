...
Companion documents: [PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) (why) Â·
[ARCHITECTURE.md](ARCHITECTURE.md) (how) Â· [MEMORY.md](MEMORY.md) (durable facts).
...
A general **pluggable module platform** â€” not vision-only. Instead of building each product as a
...
|---|---|---|---|
| **AI Supervisor** | Vision, GPU, real-time | coverage %, SOP step | SOP-compliance system. X3D-S model, trained on a small imbalanced dataset. Hardware-bound â†’ must be on-prem. |
| **Image Inspection** | Vision, GPU | pass/fail, defect | Second of the two current products. |
| **OCR** | Vision, GPU | text + confidence | P2 â€” later. |
| **Crowning** | Non-vision metrology | J2 profile extraction | Stateless request/response â†’ could run anywhere, including cloud. |
...
"Both" in the tracker means AI Supervisor **and** Image Inspection â€” the two products currently driving
...
[PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) Â§4.

**Shape**
- **Five roles:** Core (control plane) Â· Source services (produce + normalize) Â· Module services
  (products) Â· Sink services (act on results) Â· Vision Runtime (shared GPU machinery, optional).
- **Hybrid communication:** core for control/lifecycle, a **bus** (pub/sub by topic) for data.
- Every service knows only its **input/output topics** â€” never another service's identity or location.
...
- **Data standardization** lives in the source service's `normalize` stage (format + calibrate), driven
  by per-input config â€” so changing the camera doesn't change any module.

**Portability**
- Target path: **JetPack 5.1 (dev) â†’ JetPack 6 (deploy) â†’ Windows GPU PC**.
- Three bands: product code (identical everywhere) / platform layer (built once per OS) / OS+driver
  stack (a prerequisite).
- Cost tiers: **T1** module logic never changes Â· **T2** environment rebuilt once per OS in a
  Dockerfile Â· **T3** small localized code (TRT8â†’TRT10 in one backend file).
...
## 4. Exploratory â€” discussed, not decided
...
- ~~**Core scope**~~ â€” **settled 2026-07-28, moved to Â§3:** Core owns the shared-memory frame
...
1. **Vision Runtime repo shape** â€” separate repo vs. a folder in the monorepo. (Currently drawn in-tree
   but described as "its own repo/process" â€” a mild contradiction to resolve.)
2. **Startup sequence** â€” where the Vision Runtime and its port fit in the core's start/registration
   order. The documented order is sources â†’ sinks â†’ modules only.
3. **Camera SDK isolation** â€” put the camera behind a port only if/when the Windows x86 target becomes
   real. It already publishes by topic, so it can be promoted later without touching any module.

Full list, including the ones that block work: [PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) Â§9.

---

## 6. Build order

### Tier 0 â€” Contracts (schemas only, no code) â† **start here**
...
### Tier 1 â€” Skeleton loop (proves contracts hold; no real hardware or models)
Bus (in-memory pub/sub) â†’ Core (minimal boot) â†’ Mock source (fake frames) â†’ Echo module (canned result)
â†’ Log sink (prints results) â†’ Config loader.
**Goal:** frames flow mock source â†’ echo module â†’ log sink correctly. That single test proves the role
split works before touching real camera or model code.

### Tier 2 â€” Real I/O
GigE source Â· OpenCV/USB source (reuse from blockline-sequence) Â· normalize stage Â· PLC sink Â·
storage/history sink Â· per-OS Dockerfile.
...
then verifies them against its linked TensorRT) Â· GPU memory pool Â· batching scheduler Â· generic
pre/post processing Â· port layer (version firewall).

### Tier 4 â€” Modules
blockline-sequence (first real port) Â· AI Supervisor Â· Image Inspection Â· OCR Â· Crowning.

### Tier 5 â€” UI
Tauri shell + Rust bridge to Go core Â· shared design tokens Â· reusable components (StatusBadge,
CaptureCommandBar) Â· module UI panel slot.

---

## 7. Reuse inventory

**Source repos:** `blockline-sequence` (existing QC app â€” HMI-style UI, HistoryPage, PlcPage, running on
mock/simulated state) Â· `1230-biztel-app` (has real GigE camera reconnect logic).
...
- detection-result-schema (fractional-bbox detection contract, end-to-end Pythonâ†’browser)
- StatusBadge, CaptureCommandBar layout
- mock/sample frame generation

**Lift after decoupling**
- image-preprocessing (grayscaleâ†’BGR + resize normalize â€” cut out of its ABC into a free function)
- inference-runtime classical CV (blob-detect by area/aspect, hole counting â€” drop the config import,
  generic terms)
- detection-overlay-rendering (fractionâ†’pixel math + alpha-blend overlay â€” cut domain status colours)
- sequence-comparison (pairwise position-swap detection â€” return pairs instead of mutating in place)
- ui-components app shell (sidebar + clock + role picker â€” delete the fake client-side auth first)
- image-pan-zoom (wheel zoom, drag, focus-on-box â€” extract as a hook, discard the component)

**Rewrite â€” concept reusable, code isn't**
...
- model-loading Â· data-persistence Â· frame-capture-synthetic Â· HTTP glue (the seam the bus replaces
  wholesale) Â· logging-telemetry â€” all effectively build-from-scratch.
...
**P0 â€” core infra, needed by both AI Supervisor and Image Inspection**
Configuration Framework Â· Configuration Validation Â· Configuration Versioning Â· Event Framework Â·
Runtime State Framework (Starting/Running/Degraded/Error/Stopped) Â· Module Context API Â· **Model
Registry** (GPU-free catalog â€” renamed from "Model Management (lightweight)", see Â§11.2) Â· Structured
Logging Framework Â· Metrics Framework Â· Health Monitoring Framework.
...
| CI/CD Pipeline | P0 | Samarth | Makes the mock/synthetic sources actually useful â€” run on every commit |
| Deployment / Installer Packaging | P0 | Samarth | Distinct from per-OS Dockerfiles â€” this is how it physically reaches the factory floor |
...
| Audit Trail / Traceability | P1 | Samarth | Common QC/industrial compliance need â€” cheaper now than retrofitted |
| Alerting / Notification | P1 | Samarth | Health Monitoring detects; this decides who gets told and how |
| Dataset / Model Training Pipeline | P2 | Chetak | Distinct from the runtime Model Registry â€” it *produces* the artifacts the registry catalogs |
...
|---|---|---|
| **Module / Page** | The name of the thing being built â€” one deliverable per row | all 95 rows |
| **Category** | Which architectural layer or role it belongs to | all 95 rows |
| **Commonality** | Who uses it â€” shared infrastructure, or one specific product | all 95 rows |
...
| **Description** | What the item actually is, in one sentence | **19 of 95** â€” Platform Frameworks only |
| **Why Needed Now** | The justification for its priority â€” what breaks without it | **19 of 95** â€” Platform Frameworks only |
...
anywhere in the workbook** â€” those definitions are derived from
[ARCHITECTURE.md](ARCHITECTURE.md) Â§3 and Â§8.

---

### `Category` â€” which architectural layer it belongs to

Not in the Legend sheet. Maps directly onto the five roles plus three support layers.

| Value | Meaning | Rows |
|---|---|---|
| **Contracts** | The global shared language â€” envelopes, topic naming, interfaces, the inference contract. Schemas, not code. | 7 |
...
| **Source Services** | The input side â€” capture â†’ normalize â†’ publish. Cameras, mock/synthetic frames, calibration, timestamping. | 7 |
...
| **Vision Runtime** | The optional Layer 1 GPU service â€” backends, engine cache, GPU memory, batching, generic pre/post, the port layer. | 6 |
| **Sink Services** | The output side â€” PLC writer, storage/history, UI result sink, logging/telemetry. | 4 |
...
| **Cross-cutting** | Concerns with no single home â€” error handling, retry/reconnect, schema validation, the HTTP glue being retired. | 4 |

---

### `Commonality` â€” who uses it
...
The project-name values actually in use: `blockline-sequence` (4) Â· `AI Supervisor` (2) Â·
`Image Inspection` (2) Â· `OCR` (2) Â· `Crowning` (2) â€” 12 rows.

**Undocumented convention:** the remaining 5 use a **compound form** â€” `Platform-wide / blockline-sequence`
(4 rows) and `Platform-wide / 1230-biztel-app` (1 row). The Legend doesn't mention this. It reads as
*"belongs to the platform, but the existing code to start from lives in that repo"* â€” i.e. it's marking
provenance, not ownership. Worth either formalising or splitting into a separate "Source repo" column.

> âš ï¸ **This column currently carries no information beyond Category.** All 19 `Both` rows are Platform
> Frameworks and all 19 Platform Frameworks rows are `Both` â€” a perfect overlap â€” even though
> Configuration Framework, Structured Logging, and Metrics are plainly platform-wide shared infra rather
> than two-product-specific. See Â§11.1.

---

### `Status` â€” does code for this already exist
...
68 rows from zero. The per-item detail of what to lift, decouple, or rewrite is in Â§7.

---

### `Developed By` â€” who owns it

| Value | Legend definition | Rows | Colour |
|---|---|---|---|
| **Chetak** | *"CV / AI / ML â€” inference, models, image processing, vision runtime"* | 34 | blue |
| **Samarth** | *"Web / App â€” UI, bus/core, sinks, integrations, config"* | 61 | pink |

Read this as *drafting assignment*, not exclusive ownership â€” particularly for the Contracts rows, which
the design explicitly treats as owned by nobody and obeyed by everyone. See Â§11.6.

---

### `Priority` â€” when it has to exist
...
Priority is about **dependency order**, not importance or effort â€” a P2 item can still be essential to
ship, it just doesn't block anything. Note that priority alone doesn't fully order the work: several
Vision Runtime items are P1 while the P1 modules that depend on them are also P1. See Â§11.3.

### Where it stands

Per-column counts are in the tables above. The cross-cut those tables can't show â€” **who owns what, and
when it's due**:
...
- **68 of 95 rows are greenfield.** The platform is essentially unbuilt; only 7 items can be reused
  without changes.
- **Samarth holds 61 rows and 25 of the 40 P0s** â€” all 22 Pages plus 17 of the 19 Platform Frameworks.
  Chetak's 34 are the CV/GPU stack. See Â§11.5.
- **The 7 Contracts rows are all P0** and all in the "nothing exists yet" state â€” which is why Â§13 says
  to start there.
...
1. **`Commonality` isn't carrying information.** All 19 `Both` rows are Platform Frameworks, and all 19
   Platform Frameworks rows are `Both` â€” a perfect overlap. But Configuration Framework, Structured
...
2. ~~**Three overlapping model-loading items.**~~ **RESOLVED 2026-07-28.** Collapsed to two components
   plus one deletion â€” full reasoning in [ARCHITECTURE.md](ARCHITECTURE.md) Â§5.1:
   - **"Model Management (Lightweight)" â†’ rename to "Model Registry."** Keeps catalog, versioning,
     compatibility, artifact selection. **Loses "loading"** â€” that was the collision. GPU-free, stays
     **P0**, buildable now.
   - **"Engine cache" â†’ unchanged**, Vision Runtime, Layer 1, P1 with Tier 3.
   - **"Model loading (per module)" â†’ DELETE the row.** It is P3's OOM failure and violates the
     "a module never contains GPU machinery" invariant. Its legitimate part (declaring which model)
     already exists as `requires.models` in the module manifest â€” no code needed.
...
3. **A P1-depends-on-P1 ordering risk.** All 6 Vision Runtime rows are P1, and AI Supervisor /
   Image Inspection module logic are also P1 â€” but those modules *call* the Runtime for all GPU work.
...
4. **Rows the docs imply but the sheet doesn't have:**
   ...
   - **Shared-memory frame transport** â€” called out as the hot data path *and* the decision that could
     change Core's language.
   - **Echo module** from the Tier 1 skeleton loop.
   - **`blockline-sequence` module logic** â€” every other module has a logic row, and the docs call this
     one the first real port.

5. **Workload skew.** Samarth holds 61 of 95 rows and 25 of 40 P0s â€” all 22 Pages plus 17 of 19 Platform
...
6. **Contracts are individually assigned to owners**, which sits oddly against the design's "global
   ...
   spec) â€” meaning 3 of the 4 "write these first" schemas land on Chetak, who also owns the entire CV/GPU
...
These four root-level docs â€” [PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md),
[ARCHITECTURE.md](ARCHITECTURE.md), this file, and [MEMORY.md](MEMORY.md) â€” are the canonical set. They
...
shared 77-case fixture corpus. The Tier 1 skeleton loop (mock source â†’ echo module â†’ log sink) runs
and is covered by an automated test.

Remaining cheap decision with outsized downstream effect:
- Collapse the **three model-loading items** into one (Â§11.2) â€” it determines who builds what in
  Tier 3. Still open.
