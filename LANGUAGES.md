# BiztelAI Platform — Language Policy

**Status:** binding · **Last updated:** 2026-07-28

Companion documents: [ARCHITECTURE.md](ARCHITECTURE.md) (§14 is the source of this table) ·
[PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) · [CONTEXT.md](CONTEXT.md) · [MEMORY.md](MEMORY.md).
...
They coexist because the **bus contract is the only thing they share** — components exchange messages,
never code.

### Why Core is Rust, not Go

**Settled 2026-07-28: Core owns frame transport.** That closes
[PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) §9.1, and it decides the language.

A pure control plane would be Go — that was never in doubt. But a core that owns the shared-memory
...
|---|---|---|---|
| Contracts — Chetak's four | **Python** (+ JS binding) | 2026-07-28 | Real consumers are ML modules and the Vision Runtime |
| **Model Registry** | **Python** | 2026-07-28 | GPU-free, Chetak-owned, and its primary caller is the Vision Runtime. Its **catalog file is language-neutral JSON**, so a Rust core can read the same catalog later without sharing code — data, not a library, crosses that boundary. |
| Engine cache | **C++ or Python** | — | Follows the Vision Runtime; bound to CUDA/TRT |
...
|---|---|---|---|
| Frame envelope | Chetak | sources, ML modules, Vision Runtime, **core transport** | **Python** ✅ · JS ✅ · Rust ⏳ |
| Result envelope | Chetak | ML modules, sinks, UI | **Python** ✅ · JS ✅ · Rust ⏳ |
| Module interface / manifest | Chetak · Samarth | ML modules, core | **Python** ✅ · JS ✅ · Rust ⏳ |
| Inference contract | Chetak | ML modules ↔ Vision Runtime | **Python** ✅ · JS ✅ · C++ ⏳ |
| Bus topic naming | Samarth | everyone | Python ✅ · JS ✅ · Rust ⏳ |
| Service interface | Samarth | everyone | JS ✅ · Python ⏳ · Rust ⏳ |

**Python is the priority binding for Chetak's four**, because their real consumers — AI Supervisor,
Image Inspection, and the Vision Runtime — are Python. The JS binding exists because the Core API and
operator UI are currently JavaScript (see §4).
...
[PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) §7.

### Layout: no binding is primary

```
packages/contracts/
  fixtures/conformance.json   the shared truth — language-neutral
  bindings/
...
Each binding sits under `bindings/<language>/`, symmetrically. **No language gets `src/`** — that
...
Python code targets **3.8** — the JetPack 5.1 dev box — and must run unchanged on JetPack 6 (3.10) and
Windows (3.11).
...
This is the **Tier 3** cost from [ARCHITECTURE.md](ARCHITECTURE.md) §11: write to the lowest supported
version and the version delta disappears.

---

## 4. Current deviations — known and deliberate

The code in this repo does not yet match §1 everywhere. Recorded here so it is a **decision**, not
drift:

| Item | Policy says | Actually is | Status |
|---|---|---|---|
| Core API (`apps/api`) | **Rust** | **Node.js / Express** | Deviation — scaffold only. Now unblocked: the language is decided. Port when Core grows real supervision or any frame-transport logic; the JS skeleton is fine until then. |
| Bus | **Rust** (part of core) | **JS, in-process** | Deviation — Tier 1 skeleton. Ports with the core. |
...
| Echo module | Python (ML/vision) or Any (logic) | **JS** | ✅ Acceptable — `echo` is a logic/test module, and the table allows "Any" there. |
| Contracts | one binding per consuming language | **JS + Python** | ✅ Compliant. Rust binding needed when Core is ported. |

**No longer blocked.** [PROBLEM_STATEMENT.md](PROBLEM_STATEMENT.md) §9.1 is resolved: Core owns frame
...
`fixtures/conformance.json`) → bus → registry/lifecycle → shared-memory transport. The contracts
...
exactly the six rows of the table in §1 above, pasted into adjacent columns starting at row 1. They
...
**C++ or Python**, and the module interface in **C++ or Rust** — none of which is true or intended.

**This file is the authority on language, not the spreadsheet.** Either clear those three columns in
the tracker or replace them with a real per-row language field derived from §1.
